...
-- Tiempo de generación: 02-07-2024 a las 18:37:47
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_ACTUALIZAR_CORREO_CLIENTE` (IN `_ID` INT(11), IN `_CORREO` VARCHAR(200))   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_ACTUALIZAR_NOMBRE_CLIENTE` (IN `_ID` INT(11), IN `_NOMBRE` VARCHAR(200))   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_ACTUALIZAR_TELEFONO_CLIENTE` (IN `_ID` INT(11), IN `_TEL` INT(10))   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_CANCELAR_CITA` (IN `_ID_RESERVA` INT)   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_CREAR_CITA` (IN `_ID_TURNO` INT(11), IN `_ID_SERVICIO` INT(11), IN `_FECHA` DATE)   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_CREAR_FACTURA_CITA` (IN `_ID_RESERVA` INT(11), IN `_ID_CORTE` INT(11), IN `_ID_CLIENTE` INT(11), IN `_ID_BARBERO` INT(11))   INSERT INTO factura_reserva(id_reserva, id_corte,id_cliente, id_barbero)
VALUES (_ID_RESERVA, _ID_CORTE, _ID_CLIENTE,_ID_BARBERO)$$

DROP PROCEDURE IF EXISTS `SP_CREAR_RESENA`$$
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_CREAR_RESENA` (IN `_ID` INT(11), IN `_COMENTARIO` VARCHAR(255))   BEGIN
INSERT INTO resenas ( id_cliente, comentario) VALUES (_ID, _COMENTARIO);

END$$

DROP PROCEDURE IF EXISTS `SP_DESACTIVAR_CUENTA_CLIENTE`$$
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_DESACTIVAR_CUENTA_CLIENTE` (IN `_ID` INT(11))   BEGIN
UPDATE clientes
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_ELIMINAR_RESENAS` (IN `_ID` INT(11), IN `_ID_RESENA` INT(11))   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_LOGUEAR_ADMIN` (IN `_CORREO` VARCHAR(200))   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_LOGUEAR_USUARIO` (IN `_CORREO` VARCHAR(200))   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_MOSTRAR_BARBEROS` ()   BEGIN
SELECT B.id_barbero, B.nombre, B.telefono, B.contrasena, B.correo
FROM barberos B;
END$$

DROP PROCEDURE IF EXISTS `SP_MOSTRAR_CALIFICACION`$$
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_MOSTRAR_CALIFICACION` ()   BEGIN 
SELECT C.id_cliente, C.comentario
FROM calificacion C;
END$$

DROP PROCEDURE IF EXISTS `SP_MOSTRAR_CITAS`$$
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_MOSTRAR_CITAS` (IN `_ID` INT(11))   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_MOSTRAR_CORTES` ()   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_MOSTRAR_CORTES_SERVICIO` (IN `_ID_SERVICIO` INT(11))   BEGIN
SELECT C.id_corte ,C.nombre, C.id_servicio
FROM cortes C
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_MOSTRAR_PRODUCTOS` ()   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_MOSTRAR_PROMOCIONES` ()   BEGIN
SELECT p.codigo ,p.descripcion, p.codigo
FROM promociones p;
END$$

DROP PROCEDURE IF EXISTS `SP_MOSTRAR_RESENAS`$$
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_MOSTRAR_RESENAS` ()   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_MOSTRAR_SERVICIOS` ()   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_MOSTRAR_TODOS_CLIENTES` ()   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_MOSTRAR_TURNO` ()   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_MOSTRAR_UN_BARBERO` (IN `_ID` INT(11))   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_MOSTRAR_UN_CLIENTE` (IN `_ID` INT(11))   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_MOSTRAR_UN_CORTE` (IN `_ID` INT(11))   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_MOSTRAR_UN_PRODUCTO` (IN `_ID` INT)   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_REGISTRAR_BARBERO` (IN `_ID` INT(11), IN `_NOMBRE` VARCHAR(200), IN `_TELEFONO` INT(10), IN `_CONTRA` VARCHAR(200), IN `_CORREO` VARCHAR(200))   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_REGISTRAR_CLIENTE` (IN `_ID` INT(11), IN `_NOMBRE` VARCHAR(200), IN `_TELEFONO` INT(10), IN `_CORREO` VARCHAR(200), IN `_CONTRA` VARCHAR(200))   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_REPROGRAMAR_CITA` (IN `_ID_RESERVA` INT(11), IN `_ID_HORARIO` INT(11), IN `_FECHA` DATE)   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_VERIFICAR_CUENTA_CLIENTE` (IN `_ID` INT)   BEGIN
...
CREATE DEFINER=`supraproject`@`%` PROCEDURE `SP_VERIFICAR_EXISTENCIA_DE_CUENTA` (IN `_ID` INT(11), IN `_CORREO` VARCHAR(255))   BEGIN 
...
(989, 'Freddy Lopez', 8994, '$2b$04$YUVRbeI76Vhq0WKSZ79houoxHOYSwSS8QVqxnADxe6Zb.4IQ84rxy', 'Llanta400@gmail.com', 'ACTIVO'),
...
(1020405548, 'Angie NoHava', 27827323, '$2b$04$aJb79fpXfKt3oDSkgSQiGeLstU/YuKBxVRDGAVdsv863V1rsiY8AK', 'angienohava03@gmail.com', 'ACTIVO');
...
--
ALTER TABLE `barberos`
  MODIFY `id_barbero` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1000414628;
...
--
ALTER TABLE `factura_reserva`
  MODIFY `id_factura_reserva` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=43;
...
--
ALTER TABLE `resenas`
  MODIFY `id_resena` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=18;

--
-- AUTO_INCREMENT de la tabla `reservas`
--
ALTER TABLE `reservas`
  MODIFY `id_reserva` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=66;
